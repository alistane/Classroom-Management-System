...
private:
	...
	std::string academic_term; // e.g spring 2026, 
	Student students[10];
	Teacher teachers[10];
public:
	std::string courses[5] = {"Computer Science", "Maths", "Urdu", "Islamiat", "Social Studies"};
	void set_classroom_name(std::string name);
	void set_classroom_term(std::string name);
	std::string get_classname();
	std::string get_class_term();
